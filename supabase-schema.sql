...
create table if not exists public.rehab_profiles (
...
alter table public.rehab_profiles enable row level security;

drop policy if exists "Users can read own rehab profile" on public.rehab_profiles;
create policy "Users can read own rehab profile"
  on public.rehab_profiles
  for select
  using (auth.uid() = user_id);

drop policy if exists "Users can insert own rehab profile" on public.rehab_profiles;
create policy "Users can insert own rehab profile"
  on public.rehab_profiles
  for insert
  with check (auth.uid() = user_id);

drop policy if exists "Users can update own rehab profile" on public.rehab_profiles;
create policy "Users can update own rehab profile"
  on public.rehab_profiles
  for update
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

drop policy if exists "Users can delete own rehab profile" on public.rehab_profiles;
create policy "Users can delete own rehab profile"
  on public.rehab_profiles
  for delete
  using (auth.uid() = user_id);
